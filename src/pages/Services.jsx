...
const FAQItem = ({ faq }) => {
  ...
  return (
    <div className="border border-[var(--foreground)]/10 bg-[var(--background)] rounded-2xl overflow-hidden transition-all duration-300 hover:border-[var(--color-terracotta)]/50">
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className="w-full px-6 py-6 flex items-center justify-between text-left focus:outline-none"
      >
        <h4 className={`text-lg font-bold font-heading transition-colors ${isOpen ? 'text-[var(--color-terracotta)]' : 'text-[var(--foreground)]'}`}>
          {faq.q}
        </h4>
        <div className={`text-[var(--color-terracotta)] transition-transform duration-300 ${isOpen ? 'rotate-180' : 'rotate-0'}`}>
          <ChevronDown size={20} />
        </div>
      </button>
      <AnimatePresence>
  ...
  if (isSubmitted) {
    return (
      <div className="w-full p-8 text-center">
        <Sparkles className="text-[var(--color-terracotta)] mx-auto mb-4" size={32} />
  ...
  return (
    <div className="w-full">
      <div className="text-center mb-6">
        <h3 className="text-2xl font-bold text-[var(--color-terracotta)] font-heading mb-2">Instant Booking</h3>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <input 
          ...
          className="w-full bg-[var(--background)] border border-[var(--foreground)]/10 rounded-lg px-4 py-3 text-[var(--foreground)] placeholder-[var(--foreground)]/40 focus:outline-none focus:border-[var(--color-terracotta)] transition-colors"
        />
        <input 
          type="tel" 
        ...
        <select 
          ...
          className="w-full bg-[var(--background)] border border-[var(--foreground)]/10 rounded-lg px-4 py-3 text-[var(--foreground)] focus:outline-none focus:border-[var(--color-terracotta)] transition-colors"
        >
        ...
        <button 
          type="submit" 
          className="w-full bg-[var(--color-terracotta)] text-white font-bold uppercase tracking-widest text-sm py-4 rounded-lg hover:bg-amber-400 transition-colors mt-2"
        >
          Request Appointment
        </button>
      </form>
  ...
  return (
    <div className="w-full bg-[var(--background)] min-h-screen">
        ...
        <div className="relative z-10 text-center px-6 max-w-5xl mx-auto mt-0">
          ...
          >
            ...
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-black font-heading leading-tight mb-8">
              The Architecture <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[var(--color-terracotta)] to-amber-200">Of Destiny</span>
            </h1>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-6 py-3 rounded-full text-sm font-bold uppercase tracking-wider transition-all duration-300 border ${
                      activeCategory === cat.id 
                        ? 'bg-[var(--color-terracotta)] text-white border-[var(--color-terracotta)] shadow-[0_0_20px_rgba(212,175,55,0.4)]' 
                        : 'bg-transparent text-[var(--foreground)] border-[var(--foreground)]/10 hover:border-[var(--color-terracotta)]/50 hover:bg-[var(--foreground)]/5'
                    }`}
                  >
                {cat.icon}
                {cat.label}
            ...
            {activeCategory === 'all' && (
              ...
              >
                <div className="text-center mb-12">
                  <h2 className="text-3xl md:text-4xl font-bold font-heading">Signature <span className="text-[var(--color-terracotta)]">Interventions</span></h2>
                </div>
                
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  {/* Featured Large Card */}
                  <Link to={`/services/${signatureServices[0].id}`} className="group lg:col-span-2 relative h-[400px] lg:h-[500px] rounded-3xl overflow-hidden border border-[var(--color-terracotta)]/20 shadow-2xl block">
                    ...
                    <div className="absolute bottom-4 left-4 right-4 p-6 md:p-8 z-20 bg-[var(--background)]/90 backdrop-blur-md rounded-2xl">
                      <div className="text-[var(--color-terracotta)] uppercase tracking-widest text-xs font-bold mb-2">Most Requested</div>
                      <h3 className="text-3xl md:text-4xl font-black font-heading mb-2 text-[var(--foreground)] group-hover:text-[var(--color-terracotta)] transition-colors">{signatureServices[0].title}</h3>
                      <p className="text-base opacity-80 max-w-2xl text-[var(--foreground)] line-clamp-2 mb-4">{signatureServices[0].desc}</p>
                      <div className="inline-flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-[var(--color-terracotta)]">
                  ...
                  <div className="flex flex-col gap-6">
                    {signatureServices.slice(1).map((service, idx) => (
                      <Link key={idx} to={`/services/${service.id}`} className="group relative flex-1 min-h-[200px] lg:min-h-0 rounded-3xl overflow-hidden border border-[var(--color-terracotta)]/20 shadow-xl block">
                        ...
                        <div className="absolute bottom-3 left-3 right-3 p-4 z-20 bg-[var(--background)]/90 backdrop-blur-md rounded-xl">
                          <h3 className="text-xl font-bold font-heading mb-1 text-[var(--foreground)] group-hover:text-[var(--color-terracotta)] transition-colors">{service.title}</h3>
                          <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-[var(--color-terracotta)]">
                ...
                {displayServices.map((service, index) => (
                  ...
                  >
                    <Link
                      ...
                      className="block group relative h-full rounded-[2rem] overflow-hidden border border-[var(--color-terracotta)]/20 shadow-[0_5px_20px_rgba(0,0,0,0.5)] bg-[var(--color-cream)] hover:border-[var(--color-terracotta)]/50 hover:shadow-[0_0_30px_rgba(212,175,55,0.15)] transition-all duration-500"
                    >
                      {/* Image header */}
                      ...
                      <div className="p-6 md:p-8 flex flex-col relative z-20 -mt-12">
                        <div className="w-12 h-12 rounded-full bg-[var(--background)] border border-[var(--color-terracotta)]/30 flex items-center justify-center mb-6 text-[var(--color-terracotta)] group-hover:bg-[var(--color-terracotta)] group-hover:text-white transition-colors duration-300">
                          {serviceIconMap[service.id] || <Sparkles size={20} />}
                        </div>
                        <h3 className="text-xl font-bold font-heading mb-3 text-[var(--foreground)] group-hover:text-[var(--color-terracotta)] transition-colors">{service.title}</h3>
                        <p className="text-sm opacity-60 leading-relaxed mb-6 line-clamp-3">{service.desc}</p>
                        
                        <div className="mt-auto inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[var(--color-terracotta)] opacity-80 group-hover:opacity-100">
      ...
      <section className="pt-8 pb-8 md:pb-24 lg:pt-12 relative overflow-hidden flex items-center justify-center border-y border-[var(--foreground)]/5 bg-[var(--background)]">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-[var(--color-terracotta)]/5 via-transparent to-transparent"></div>
        
        <div className="relative z-10 w-full max-w-4xl mx-auto px-6">
          <div className="text-center mb-16">
            <div className="text-[var(--color-terracotta)] tracking-[0.3em] uppercase text-sm mb-4 font-semibold">Answers</div>
            <h2 className="text-3xl md:text-5xl font-bold font-heading">Frequently Asked <span className="text-[var(--color-terracotta)]">Questions</span></h2>
          </div>
        ...
        <div className="relative z-10 w-full max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="text-center lg:text-left">
            <h2 className="text-4xl md:text-6xl font-black font-heading mb-8">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[var(--color-terracotta)] to-amber-200">Not Fixed.</span>
            </h2>
