    ...
    return (
      <div className="min-h-screen flex items-center justify-center pt-24">
        <div className="text-center">
          ...
          <button aria-label="Interactive Button" onClick={() => navigate('/services')} className="text-[var(--color-terracotta)] hover:underline">
            Return to Services
          </button>
        </div>
      </div>
        ...
        <div className="absolute inset-0 z-20 flex items-end pb-12 md:pb-24">
          <div className="container mx-auto px-6 max-w-7xl flex justify-end">
            <motion.div
              ...
              className="max-w-2xl bg-[var(--background)]/95 backdrop-blur-md p-8 md:p-12 rounded-3xl border border-[var(--color-terracotta)]/20 shadow-xl"
            >
      ...
      <div className="container mx-auto px-6 max-w-7xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-12 mb-12 lg:mb-24">
          ...
          <div className="lg:col-span-2">
            ...
            >
              <h2 className="text-3xl font-bold mb-6 border-b border-[var(--color-antique-gold)]/10 pb-4">About This Service</h2>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {service.testimonials.map((testi, idx) => (
                  <div key={idx} className="glass-card p-6 rounded-2xl relative border border-[var(--color-deep-brown)]">
                    <div>
                      <Star className="text-[var(--color-terracotta)] mb-4" size={24} />
                      <p className="text-[var(--foreground)] mb-6 italic">"{testi.text}"</p>
                    </div>
                    <div className="flex items-center gap-3 border-t border-[var(--color-antique-gold)]/10 pt-4 mt-auto">
                      <div className="w-10 h-10 rounded-full overflow-hidden shrink-0 border border-[var(--color-sand)]">
          ...
          <div className="lg:col-span-1 space-y-8">
            ...
            <div className="glass p-8 rounded-3xl border border-[var(--color-deep-brown)]">
              ...
              <ul className="space-y-4 mb-8 relative z-10">
                {service.badges.map((badge, idx) => (
                  <li key={idx} className="flex items-center gap-3 text-[var(--foreground)]">
                    <CheckCircle size={20} className="text-[var(--color-terracotta)]" />
                    <span>{badge}</span>
                  </li>
                ))}
              </ul>

              <button aria-label="Interactive Button" className="w-full py-4 rounded-full bg-[var(--color-terracotta)] text-[#FFFFFF] font-bold text-lg hover:shadow-[0_0_20px_rgba(99, 102, 241, 0.5)] transition-all duration-300 relative z-10">
                Consult Now
              </button>
            </div>

            {/* Functionality: Quick Inquiry Form */}
            <div className="glass p-8 rounded-3xl border border-[var(--color-antique-gold)]/10">
              ...
              <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                <div>
                  <div className="flex items-center gap-2 bg-[#C85A3F]/40 rounded-xl px-4 py-3 border border-[var(--color-antique-gold)]/10 focus-within:border-[var(--color-deep-brown)]">
                ...
                <div>
                  <div className="flex items-start gap-2 bg-[#C85A3F]/40 rounded-xl px-4 py-3 border border-[var(--color-antique-gold)]/10 focus-within:border-[var(--color-deep-brown)]">
                ...
                <button aria-label="Interactive Button" className="w-full py-3 rounded-xl border border-[var(--color-deep-brown)]">
                  <Send size={18} />
                  Send Message
                </button>
              </form>
        ...
        >
          ...
          <div className="space-y-4">
            {service.faqs.map((faq, idx) => (
              <div
                key={idx}
                className={`glass border rounded-2xl overflow-hidden transition-colors duration-300 ${activeFaq === idx ? 'border-[var(--color-deep-brown)]' : 'border-[var(--color-antique-gold)]/10'}`}
              >
                ...
                >
                  ...
                  {activeFaq === idx ? (
                    <ChevronUp className="text-[var(--color-terracotta)]" />
                  ) : (
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <div className="flex justify-between items-end mb-8 border-b border-[var(--color-antique-gold)]/10 pb-4">
            <h2 className="text-3xl font-bold">You May Also Like</h2>
            <Link to="/services" className="text-[var(--color-terracotta)] hover:underline flex items-center gap-1 text-sm font-semibold uppercase tracking-wider">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
            {recommendedServices.map((rec) => (
              <Link
                key={rec.id}
                to={`/services/${rec.id}`}
                className="group block relative glass-card rounded-2xl overflow-hidden border border-[var(--color-antique-gold)]/10 hover:border-[var(--color-deep-brown)]">
                ...
                <div className="p-6">
                  <h3 className="text-xl font-bold mb-2 group-hover:text-[var(--color-terracotta)] transition-colors">{rec.title}</h3>
