    ...
    <section className="relative min-h-screen pt-8 pb-8 md:pb-24 lg:pt-12 lg:pb-24 px-6 md:px-12 lg:px-24 flex items-center bg-[var(--background)] overflow-hidden">
      ...
      <div className="container mx-auto max-w-7xl relative z-10 flex flex-col lg:flex-row gap-16 lg:gap-24">
        ...
        <div className="w-full lg:w-1/3 lg:sticky lg:top-32 h-fit">
          <div className="text-[var(--color-terracotta)] font-semibold tracking-[0.2em] uppercase text-sm mb-4 flex items-center gap-4">
          ...
          <h2 className="text-4xl md:text-6xl font-bold mb-8 tracking-tight font-heading leading-tight">
            Curated <br/> <span className="text-[var(--color-terracotta)] text-glow">Excellence</span>
          </h2>
          ...
          <div className="grid grid-cols-2 gap-6 mb-12">
            <div className="bg-[var(--color-terracotta)]/5 border border-[var(--color-terracotta)]/20 rounded-xl p-4 transition-transform hover:-translate-y-1 duration-300">
              <span className="block text-3xl font-heading font-bold text-[var(--color-terracotta)] mb-1">99%</span>
              <span className="block text-xs uppercase tracking-wider text-[var(--foreground)] opacity-70">Success Rate</span>
            </div>
            <div className="bg-[var(--color-terracotta)]/5 border border-[var(--color-terracotta)]/20 rounded-xl p-4 transition-transform hover:-translate-y-1 duration-300">
              <span className="block text-3xl font-heading font-bold text-[var(--color-terracotta)] mb-1">24/7</span>
              <span className="block text-xs uppercase tracking-wider text-[var(--foreground)] opacity-70">Spiritual Support</span>
            </div>
            <div className="bg-[var(--color-terracotta)]/5 border border-[var(--color-terracotta)]/20 rounded-xl p-4 transition-transform hover:-translate-y-1 duration-300">
              <span className="block text-3xl font-heading font-bold text-[var(--color-terracotta)] mb-1">10k+</span>
              <span className="block text-xs uppercase tracking-wider text-[var(--foreground)] opacity-70">Lives Changed</span>
            </div>
            <div className="bg-[var(--color-terracotta)]/5 border border-[var(--color-terracotta)]/20 rounded-xl p-4 transition-transform hover:-translate-y-1 duration-300">
              <span className="block text-3xl font-heading font-bold text-[var(--color-terracotta)] mb-1">100%</span>
          ...
          <Link 
            to="/services"
            className="inline-flex items-center justify-center gap-3 px-8 py-4 bg-transparent text-[var(--foreground)] border border-[var(--color-terracotta)]/50 hover:border-[var(--color-terracotta)] hover:bg-[var(--color-terracotta)] hover:text-white font-bold uppercase tracking-wider text-sm transition-all duration-300 group rounded-lg"
          >
            View All Services
            <ArrowRight size={16} className="text-[var(--color-terracotta)] group-hover:text-white group-hover:translate-x-1 transition-all" />
          </Link>
        </div>
          ...
          {services.map((service, index) => {
            ...
            return (
              <div 
                ...
                className={`group border-b border-[var(--color-terracotta)]/20 py-10 cursor-pointer transition-all duration-500 ${
                  isActive ? 'pl-8 border-[var(--color-terracotta)]' : 'hover:pl-4'
                }`}
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-start gap-8">
                    <span className={`text-xl font-heading transition-colors duration-500 ${
                      isActive ? 'text-[var(--color-terracotta)]' : 'text-[var(--foreground)]/30 group-hover:text-[var(--color-terracotta)]/50'
                    }`}>
                      {service.id}
                    </span>
                    <div>
                  ...
                  <div className={`hidden md:flex items-center justify-center w-14 h-14 rounded-full border transition-all duration-500 ${
                    isActive ? 'border-[var(--color-terracotta)] bg-[var(--color-terracotta)] text-[var(--background)]' : 'border-transparent text-[var(--foreground)]/30 group-hover:border-[var(--color-terracotta)]/50'
                  }`}>
